...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestCounterListInspectorGUI
    {
        ...
        private void DrawSelectedUIContent()
        {
            ...
            DrawWarningIfCounterUsesTextTable(m_selectedCounter);
            EditorGUILayout.PropertyField(m_selectedCounter);
            EditorGUI.indentLevel--;
        }

        private void DrawWarningIfCounterUsesTextTable(SerializedProperty m_selectedCounter)
        {
            if (m_selectedCounter == null) return;
            var nameProperty = m_selectedCounter.FindPropertyRelative("m_name");
            if (nameProperty == null) return;
            var textTableProperty = nameProperty.FindPropertyRelative("m_textTable");
            if (textTableProperty == null) return;
            if (textTableProperty.objectReferenceValue != null)
            {
                EditorGUILayout.HelpBox("Counter names should not use localized text table values.", MessageType.Warning);
            }
        }
    }
}
