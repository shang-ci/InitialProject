...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestConditionSet
    {
        ...
        {
            if (!areConditionsMet) return false;
            int numTrue = 0;
            foreach (var condition in conditionList)
            {
                if (condition.Recheck())
                {
                    numTrue++;
                    // If we've met the minimum, succeed:
                    if (conditionCountMode == ConditionCountMode.Any) return true;
                    else if (conditionCountMode == ConditionCountMode.Min && numTrue >= minConditionCount) return true;
                }
                else if (conditionCountMode == ConditionCountMode.All)
                {
                    // If all conditions must be true and this one isn't, fail:
                    return false; 
                }
            }
