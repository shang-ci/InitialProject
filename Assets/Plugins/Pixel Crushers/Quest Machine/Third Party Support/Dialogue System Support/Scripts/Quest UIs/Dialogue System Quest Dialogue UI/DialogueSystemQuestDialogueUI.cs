...
namespace PixelCrushers.QuestMachine.DialogueSystemSupport
{
    ...
    public class DialogueSystemQuestDialogueUI : MonoBehaviour, IQuestDialogueUI
    {
        ...
        protected virtual void SetQuestMachineDialogueSystemVariables(QuestParticipantTextInfo speaker, Transform actor, Conversation conversation)
        {
            ...
            {
                if (conversation.id == -1) return; // -1 is fake warmup conversation.
        ...
        protected virtual void AddResponseNode(Conversation conversation, DialogueEntry npcNode, Quest quest)
        {
            ...
            if (quest != null)
            {
                ...
                // Check if quest has conversation content:
                List<QuestContent> content = null;
                switch (quest.GetState())
                {
                    case QuestState.WaitingToStart:
                        content = quest.offerContentList;
                        break;
                    case QuestState.Active:
                        content = quest.GetContentList(QuestContentCategory.Dialogue);
                        break;
                }
                var conversationContent = (content == null) ? null
                    : content.Find(x => x is DialogueSystemConversationQuestContent) as DialogueSystemConversationQuestContent;

                // Add link to quest conversation:
                var addedLinkToQuestConversation = false;
                if (conversationContent != null)
                {
                    var questConversation = DialogueManager.masterDatabase.GetConversation(conversationContent.conversation);
                    if (questConversation != null)
                    {
                        foreach (var destinationLink in conversation.GetFirstDialogueEntry().outgoingLinks)
                        {
                            node.outgoingLinks.Add(new Link(destinationLink));
                        }
                    }
                }
                // If we didn't add a link to quest conversation, use Lua function to call base select handler:
                if (!addedLinkToQuestConversation)
                {
