...
namespace PixelCrushers.QuestMachine.DialogueSystemSupport
{
    ...
    public class QuestDatabaseToDialogueDatabaseWindow : EditorWindow
    {
        ...
        [SerializeField]
        private bool m_copyQuestEntries = false;
        ...
        private void OnGUI()
        {
            ...
            m_copyQuestEntries = EditorGUILayout.Toggle(new GUIContent("Copy Quest Entries", "Copy QM quest nodes into DS quest entries."), m_copyQuestEntries);
        ...
        private int CopyDatabase(QuestDatabase qdb, bool report)
        {
            ...
            {
                var quest = qdb.questAssets[i];
                if (quest == null) continue;
                AddQuestIDToDatabase(quest);
                total++;
            }
        ...
        private void AddQuestIDToDatabase(Quest quest)
        {
            ...
            Item item = dialogueDatabase.GetItem(questID);
            if (item == null)
            {
                item = template.CreateItem(GetNextItemID(), questID);
                item.IsItem = false;
                dialogueDatabase.items.Add(item);
            }
            if (m_copyQuestEntries)
            {
                Field.SetValue(item.fields, "Entry Count", quest.nodeList.Count);
                for (int i = 0; i < quest.nodeList.Count; i++)
                {
                    var id = quest.nodeList[i].id.ToString();
                    if (string.IsNullOrEmpty(id)) continue;
                    Field.SetValue(item.fields, "Entry " + i, id);
                }
            }
